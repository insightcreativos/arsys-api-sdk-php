...
abstract class AbstractWrapper {
	...
	/**
	 * Get the Arsys client instance to use only one http client.
	 *
	 * @param \Arsys\API\API $master Arsys client instance
	 */
	public function __construct( \Arsys\API\API $master ) {
		$this->master = $master;
	}
	 ...
	 *
	 * @return string|array|Arsys\API\Response\Response
	 */
	 ...
	 * @return \Arsys\API\Response\Response
	 */
	protected function execute( $url, array $params = [], array $map = [], string $method = 'GET' ) {
		$errors = [];
		...
		$output = $this->output( $this->master->call( $url, $params, $method ) );
		...
		if ( $this->master->getOption( 'versionCheck' ) && ! is_null( $output->getVersion() ) ) {
			...
			if ( $this->master->api_version_minor < $version[1] && $this->master->api_version_major == $version[0] ) {
				trigger_error( 'This client is not up to date. It\'s recommended to update it to match the API version being used.',
					E_USER_WARNING );
			}
	...
	protected function validate( array $params = [], array $map = [] ) {
		...
		foreach ( $map as $key => $parameter ) {
			$hasNotParam = ! array_key_exists( $parameter['name'],
					$params ) || ( ! is_bool( $params[ $parameter['name'] ] ) && empty( $params[ $parameter['name'] ] ) );
					...
					switch ( $parameter['type'] ) {
						...
						case 'int':
							...
							if ( isset( $parameter['min'] ) && $value < intval( $parameter['min'] ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be %s or more', $parameter['name'],
									$parameter['min'] );
							}

							if ( isset( $parameter['max'] ) && $value > intval( $parameter['max'] ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be %s or less', $parameter['name'],
									$parameter['max'] );
							}
							break;
						...
						case 'email':
							if ( ! filter_var( $value, FILTER_VALIDATE_EMAIL ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be a valid email address',
									$parameter['name'] );
							}
							break;
						...
						case 'phone':
							if ( ! preg_match( '/^\+(\d+)\.(\d+)$/i', $value ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be a valid phone number, in +DD.DDDDDDDD... format',
									$parameter['name'] );
							}
							break;
						...
						case 'countryCode':
							if ( ! preg_match( "/^([A-Z][A-Z])$/i", $value ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be a valid country code',
									$parameter['name'] );
							}
							break;
						...
						case 'date':
							if ( ! preg_match( "/^[0-9]{4}(-?)(0[1-9]|1[0-2])(-?)(0[1-9]|[1-2][0-9]|3[0-1])$/i",
								$value ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be a valid date, in YYYYMMDD or YYYY-MM-DD format',
									$parameter['name'] );
							}
							break;
						...
						case 'ipv4':
							...
							if ( ! filter_var( $value, FILTER_VALIDATE_IP, $options ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be a valid IPv4 address',
									$parameter['name'] );
							}
							break;
						...
						case 'ipv6':
							...
							if ( ! filter_var( $value, FILTER_VALIDATE_IP, $options ) ) {
								$errors[] = sprintf( 'Parameter "%s" must be a valid IPv6 address',
									$parameter['name'] );
							}
							break;
					}
