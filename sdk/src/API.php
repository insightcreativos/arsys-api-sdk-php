...
class API {
	 ...
	 */
	protected $options = [
		'endpoint'     => 'https://domain.apitool.info/v2',
		'port'         => 443,
		'apiuser'      => '',
		'apipasswd'    => '',
		'autoValidate' => true,
	 ...
	 *
	 * @throws \Arsys\API\Exceptions\Error if no user or password is present
	 */
	public function __construct( array $options = [] ) {
		...
		if ( empty( $this->options['apiuser'] ) ) {
			throw new \Arsys\API\Exceptions\Error( 'You must provide an user and a password for the API' );
		}

		//Modules
//		$this->account = new \Arsys\API\Wrappers\Account( $this );
//		$this->contact = new \Arsys\API\Wrappers\Contact( $this );
		$this->domain = new \Arsys\API\Wrappers\Domain( $this );
//		$this->tool    = new \Arsys\API\Wrappers\Tool( $this );
//		$this->service = new \Arsys\API\Wrappers\Service( $this );
//		$this->ssl     = new \Arsys\API\Wrappers\SSL( $this );
//		$this->user    = new \Arsys\API\Wrappers\User( $this );
	}
	...
	public function getClient() {
		if ( is_null( $this->client ) ) {
			$this->client = new \Arsys\API\Client\Client( [
	 ...
	 * @return array
	 */
	public function call( $url, array $args = [], $method = 'GET' ) {
		...
		return $this->getClient()->execute( $url, $params, $method );
	}
	...
	public function info() {
		...
		printf( " Throw exceptions:\t\t%s" . PHP_EOL,
		...
		try {
			...
			if ( ! $uriCheck ) {
				$errors[] = "API URI cannot be blank. Check your API URI on https://www.arsys.es/admin/account/api/";
			}

			if ( ! $portCheck ) {
				$errors[] = "API Port cannot be blank. Check your API Port on https://www.arsys.es/admin/account/api/";
			}

			if ( ! $userCheck ) {
				$errors[] = "API Username cannot be blank. Check your API Username on https://www.arsys.es/admin/account/api/";
			}

			if ( ! $passCheck ) {
				$errors[] = "API Password cannot be blank. Set your API Password on https://www.arsys.es/admin/account/api";
			}
			...
			if ( count( $errors ) > 0 ) {
				...
				throw new \Exception( 'Please, fix the indicated errors before using ArsysAPI.' );
			}
			...
			if ( ! $this->getOption( 'throwExceptions' ) && ! $response->getSuccess() ) {
				throw new \Exception(
					sprintf( "Connection failed with error (%s): %s ", $response->getErrorCode(),
						$response->getErrorCodeMsg() )
				);
			}
